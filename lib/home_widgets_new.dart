...
class _SearchScreenState extends State<SearchScreen> {
  ...
  final pharmacies = const [
    {
      'name': 'Pharmacie des anges',
      'status': 'Ouverte',
    },
    {
      'name': 'Pharmacie de essos',
      'status': 'Ferme dans 45 min',
    },
    {
      'name': 'Pharmacie du ministere',
      'status': 'Ouverte 24/7',
    },
  ];
  ...
  List<Map<String, String>> get filteredPharmacies {
    ...
    return pharmacies
        .where((pharmacy) => pharmacy['name']!.toLowerCase().contains(query.toLowerCase()))
        .toList();
  }
  ...
  Future<void> _loadRemoteDoctors() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Impossible de charger les médecins depuis l’API.')),
      );
    } finally {
      setState(() {
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton.icon(
              ...
              label: Text(isLoadingRemote ? 'Chargement...' : 'Actualiser les médecins'),
            ),
            ...
            Row(
              ...
              children: [
                _IconCard(icon: Icons.local_hospital, label: 'Urgence', onTap: () => _setCategory('Urgences')),
                _IconCard(icon: Icons.local_pharmacy, label: 'Pharmacie', onTap: () => _setCategory('Pharmacie')),
                _IconCard(icon: Icons.home, label: 'À domicile', onTap: () => _setCategory('Généraliste')),
              ],
            ),
            const SizedBox(height: 24),
            const Text(
...
class _IconCard extends StatelessWidget {
  ...
  const _IconCard({required this.icon, required this.label, required this.onTap});
...
class _DoctorCard extends StatelessWidget {
  ...
  const _DoctorCard({required this.name, required this.specialty, required this.availability, required this.onReserve});
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(availability, style: const TextStyle(color: Colors.green, fontWeight: FontWeight.w600)),
              ],
            ),
          ),
          ElevatedButton(
...
class AppointmentBookingScreen extends StatefulWidget {
  ...
  const AppointmentBookingScreen({required this.doctorName, required this.specialty, super.key});
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(widget.doctorName, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text(widget.specialty, style: const TextStyle(color: Colors.black54)),
            const SizedBox(height: 24),
            const Text('Choisis un créneau', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            ...slots.map((slot) {
            ...
            ElevatedButton(
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Rendez-vous confirmé le $selectedSlot avec ${widget.doctorName}.')),
                );
...
class _TriageScreenState extends State<TriageScreen> {
  final symptoms = const ['Fièvre', 'Toux', 'Mal de tête', 'Frotement', 'Fatigue'];
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Triage IA', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text(
            ...
            const SizedBox(height: 20),
            if (step == 0) ...[
              const Text('Quel symptôme ressens-tu ?', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 10,
                runSpacing: 10,
                children: symptoms.map((symptom) {
                  ...
                  return ChoiceChip(
                    label: Text(symptom),
                    selected: selected,
                    onSelected: (_) => setState(() => selectedSymptom = symptom),
                  );
                }).toList(),
              ),
            ] else if (step == 1) ...[
              const Text('Quelle intensité ?', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 10,
                runSpacing: 10,
                children: severities.map((severity) {
                  ...
                  return ChoiceChip(
                    label: Text(severity),
                    selected: selected,
                    onSelected: (_) => setState(() => selectedSeverity = severity),
                  );
                }).toList(),
              ),
            ] else if (step == 2) ...[
              const Text('Depuis combien de temps ?', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 10,
                runSpacing: 10,
              ...
              Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Conseil', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.blue.shade900)),
                    const SizedBox(height: 12),
                    Text(advice, style: const TextStyle(fontSize: 16, color: Colors.black87)),
                  ],
                ),
              ),
              const SizedBox(height: 20),
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Suivi des consultations', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text('Retrouve ton historique et tes prochains rendez-vous.',
                style: TextStyle(fontSize: 16, color: Colors.black54)),
            const SizedBox(height: 20),
            const _AppointmentCard(
              title: 'Consultation avec Dr. ongolo Amélie',
            ...
            const SizedBox(height: 24),
            const Text('Conseils santé', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            const _InfoTile(
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Dossier médical', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            const Text('Toutes tes informations de santé en un seul endroit.',
                style: TextStyle(fontSize: 16, color: Colors.black54)),
            const SizedBox(height: 20),
            ...
            const SizedBox(height: 24),
            const Text('Documents importants', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
            const SizedBox(height: 12),
            const _DocumentCard(title: 'Ordonnance récente', subtitle: 'Télécharger'),
            const _DocumentCard(title: 'Résultat analyse sanguine', subtitle: 'Télécharger'),
          ],
        ),
      ),
    );
...
class _AppointmentCard extends StatelessWidget {
  ...
  const _AppointmentCard({required this.title, required this.subtitle, required this.status});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(status, style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.w700)),
        ],
      ),
    );
  }
